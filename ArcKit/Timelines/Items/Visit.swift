...
open class Visit: TimelineItem {
    ...
    static var minimumKeeperDuration: TimeInterval = 60 * 2
    static var minimumValidDuration: TimeInterval = 10

    static var minimumRadius: CLLocationDistance = 10
    static var maximumRadius: CLLocationDistance = 150
    ...
    public override func contains(_ location: CLLocation, sd: Double? = 4) -> Bool {
        guard let center = center, let sd = sd else {
            return false
        }
    ...
    public override func cleanseEdge(with path: Path) -> LocomotionSample? {
        ...
        if !pathEdgeIsInside && !visitEdgeIsInside {
            path.add(visitEdge)
            NotificationCenter.default.post(Notification(name: .debugInfo, object: DefaultTimelineManager.highlander,
